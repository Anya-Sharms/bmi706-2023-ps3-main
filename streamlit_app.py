...
def load_data():
    ...
    cancer_df = pd.read_csv(
        "https://raw.githubusercontent.com/hms-dbmi/bmi706-2022/main/cancer_data/cancer_ICD10.csv"
    ).melt(
        id_vars=["Country", "Year", "Cancer", "Sex"],
        var_name="Age",
        value_name="Deaths",
    ...
    pop_df = pd.read_csv(
        "https://raw.githubusercontent.com/hms-dbmi/bmi706-2022/main/cancer_data/population.csv"
    ).melt(
        id_vars=["Country", "Year", "Sex"],
        var_name="Age",
        value_name="Pop",
...
countries = st.multiselect(
    ...
    options=sorted(df["Country"].unique()),  # all countries in the dataset
    default=[
        "Austria",
        "Germany",
        "Iceland",
...
cancer = st.selectbox(
    "Select Cancer Type",
    options=sorted(df["Cancer"].unique()),  # all cancer types in the dataset
    index=sorted(df["Cancer"].unique()).index("Malignant neoplasm of stomach")  # default selection
)
